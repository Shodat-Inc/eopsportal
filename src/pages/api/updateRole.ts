...
import { updateRoleValidation } from "../../../validateSchema";
import { roleRepo } from "@/helpers/api/repo/role-repo";


// Default API handler for the PUT method to handle role update operations.
export default apiHandler({
  put: handler,
});

/**
 * Handler function to process the update operation for a role.
 *
 * @param {Object} req - The incoming request object containing the role data to be updated.
...
async function handler(req: any, res: any) {
  try {
    ...
    const validation = updateRoleValidation(updateData);
    if (validation.error) {
    ...
    const updatedRole: any = await roleRepo.update(validation.value);
